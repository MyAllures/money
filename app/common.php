 ...
 * 获取图片url
 */
function get_picture_url($id = 0, $type = 'small')
{
    ...
    if (!empty($info['path'])) {
        ...
        if ($type == 'small') {
            $url = str_replace('/', '/thumb/small_', $info['path']);
        } else {
            $url = $info['path'];
        }

        return $root_url . 'upload/picture/' . $url;
    }
 ...
 * 写入第三方通道信息记录
 */
function write_log($filename, $input_contents, $type = 0)
{
    $log_path = ROOT_PATH . '/log/' . $filename . '_' . date('Ym') . '.log';
    if ($type === 0) {//-转成json
        $input_contents = json_encode_my($input_contents);
    } elseif ($type === 1) {//原样保存
        $input_contents = $input_contents;
    }
    $contents = '——|time ' . date('Y--m-d H:i:s') . '|ip ' . request()->ip() . '| ' . PHP_EOL . $input_contents . PHP_EOL;
